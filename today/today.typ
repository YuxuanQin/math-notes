...
For an arbitrary morphism $g: S -> U H$ where $S$ is a set and $H$ a group, good candidates of the solution set are those subgroups generated by $"im" g$, note that the elements of $chevron.l "im" g chevron.r$ are all of the form $g(s_1)^alpha g(s_2)^alpha dots.h.c g(s_n)^alpha$ where $alpha = plus.minus 1$ and $s_i$ are not necessarily diffferent, i.e. every element of this generated subgroup is always a finite composition of elements in $"im" g$, so the cardinality of $chevron.l "im" g chevron.r$ is bounded by $|S| + aleph_0$, for any $g$, for any $H$.

Furthur more, the number of group structures on $|S| + aleph_0$ is also bounded (by simple estimation). So, by the Axiom of Choice, we choose a group from each isomorphic class of those group structures and gather these representations up and obtain the solution set.
...
== 12-11: An application of the classification theorem of finitely generated modules
We want to prove the surface $n T^2$ is not isomorphic to $m P^2$ for any $n, m in NN$, and as usual, consulting to fundamental groups comes to mind.

However, the fundamental groups of surfaces are rather exotic and do not admit a clean "presentation" (both in math and daily English), and it's well known that deciding if two presentations of groups are identical is diffcult.

However, the classifications of modules is known and we have a handy $"ab": grp -> ab$ preserve isomorphism as a functor!

So let's check if $"ab"(pi(n T^2)) iso "ab"(pi(m P^2))$. By simple calculation we have
  $ "ab"(pi(n T^2)) iso ZZ^(plus.o n),\ "ab"(pi(m P^2)) iso ZZ^(plus.o m - 1) plus.o ZZ_2 $

Note that $ZZ^(plus.o n)$ is torsion-free while $ZZ^(plus.o m - 1) plus.o ZZ_2$ is not, this implies the original fundamental groups must not be isomorphic.

When it comes to abelian groups, the classification of modules says that all finitely generated abelian groups are isomorphic to 
  $ plus.o.big_(i <= n) ZZ_(d_i), $
where $d_i | d_(i + 1)$ and $d_i >= 0$.

If we isolate all $ZZ_0$s, the classification can be wrriten as
  $ ZZ^(plus.o k) plus.o plus.o.big_(i <= n) ZZ_(d_i), $
and now we assume $d_i != 0$, then the abelian groups is divided into torsion-free part and torsion part. And this can be used to prove two abelian groups are not isomorphic if they admits different factors, i.e different $d_i$.
