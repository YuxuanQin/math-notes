...
#proof[
  These two lifts $tilde(f)_1$ and $tilde(f)_2$ induces a map $chevron.l tilde(f)_1, tilde(f)_2 chevron.r: Y -> E times_X E$.

  Since $Delta(E)$ is clopen in $E times_X E$ (recall @diagonal), the pre-image of it under $chevron.l tilde(f)_1, tilde(f)_2 chevron.r$ is also clopen. But $Y$ is connected, so the pre-image is either $Y$ or empty.
]
...
Well, the fact is: *there are many different (some in form, some in essential) homology theories*, invented for different purposes.

The singular one is applied to any spaces, while the simplicial one is only applied to "a few" spaces, details below.

== Simplicial Homology
The simplicial homology is literally the theory of somplices in a space, 

#definition(title: "Standard simplex")[
We denote the standard $n$-simplices by $Delta_n$, defined in the Euclidean space $RR^(n + 1)$ by the formular:
$ Delta_n := {bold(x): x_0 + ... + x_n = 1, x_i > 0}, $
]
#definition(title: "Simplicial complex")[
A simplicial complex is just a compound or combination of standard simplices, in a nice way.

The formal definition is wordy and omitted.
]
#definition(title: "Simplicial homology")[
For $K$ a simplicial complex, we can define its simplicial homology as follows:

+ $C^"Simp"_n (K)$ is defined to be the free abelian group generated by all $n$-simplices in $K$;
+ $"Simp"(K)$ is defined to be the chain complex consists of all $C^"Simp"_n (K)$, (boundary maps omitted since it's wordy);
+ The simplicial $n$-homology group $H^"Simp"_n (X)$ is defined as the $n$-homology group of the chain complex $"Simp"(K)$.
]
#remark[
So, at least for now, the simplicial homology is only defined for *simplicial complex*. Since we can not talk about "simplices" in a general space.

Based people can also define the simplicial homology not only for simplicial complex, but also *delta complex* (I don't know what is it).

But, I think there is some way to extend this definition to all spaces which admits a triangulation.
]


== Singular Homology
As stated before, the simplicial one is only defined for a (rather small) class of spaces, but the singular one is defined for all spaces.

The main idea is copying the definition of the simplicial one in a celever way and modify it so we can talk about the modified "simplices" in a general space. And this is done by the concept of "shape" or "xxx object" in category theory.
...
We find the scheme of definitions of different homology theories can be summed up as follows:

+ First define a functor $alpha: "Space" -> "Ch"_bullet.op (cal(C))$, where $"Space"$ is a category consists of some kinds of spaces and $cal(C)$ is an additive category;
+ Next, apply the $n$-homology functor defined on any chain complex category $H_n: "Ch"_bullet.op (cal(C)) -> "Alg"$, where $"Alg"$ is a category consists some kind of algebras.
...
Here is an exmaple: Suppose we have a standard $2$-simplex $Delta_2$:

#align(center, diagram({
   node((0, 0), [$v_2$])
   node((-1, 1), [$v_0$])
   node((1, 1), [$v_1$])
   edge((0, 0), (-1, 1), [$e_02$], label-side: center, "<-")
   edge((-1, 1), (1, 1), [$e_01$], label-side: center, "->")
   edge((1, 1), (0, 0), [$e_12$], label-side: center, "->")
}))
