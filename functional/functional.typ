...
#proposition[
对任意 $1<= p <= oo$，如果 $p != 2$，则 $(l^p, ||dot||_p)$ 和 $L^p [0, 1]$ 均无法由内积诱导。
]
#proof[
主要思想是，如果真能由内积诱导，则平行四边形法则定然成立，利用这点导出矛盾。

取 $x = (1, 0, ...)$ 和 $y = (0, 1, ...)$，通过考察平行四边形法则，可以导出矛盾。
]

#remark[
另外，若 $p >= 1$ 且 $p != 2$，假设确能由内积诱导，则由@dream，我们知道 $L^p$ 是 Hilbert 空间，故而由 Riesz 表示定理知 $L^p$ 自反，然而我们有 $(L^p)^* iso L^q$，其中 $q$ 是 $p$ 的共轭，这两个空间不相等，矛盾！
]
...
== 弱收敛
在本小节中，我们假设 $X$ 是赋范线性空间。

借助赋范线性空间的典范等距嵌入 $iota: X inj X^(**)$，我们可以得到另一种定义形式的 “收敛”：

#definition(title: "弱收敛")[
设 ${x_n}_(n in N) subset X$，若存在 $x_0 in X$ 使得在 $X^(**)$ 中有 ${iota x_n}$ *逐点收敛到* $iota x_0$，则称 ${x_n}$ 弱收敛到 $x_0$，记为 $ x_n ->^w x_0. $
...
有时为了区分弱收敛和按范数收敛，我们把后者称为强收敛（可能不是标准术语）。
]

#definition(title: "弱列紧、弱列闭")[
设 $E subset X$，若 $E$ 中的任意序列都有弱收敛的子列, 则称 $E$ 是*弱列紧*的。若 $E$ 中的任意弱收敛序列的极限仍在 $E$ 中, 则称 $E$ 是*弱列闭*的。
]

#definition(title: [弱 $*$ 收敛])[
这是弱收敛的 $X^*$ 版本。设 ${f_n} subset X^*$，若存在 $f_0 in X^*$ 使得 ${f_n}$ *逐点收敛到* $f_0$，即对任意 $x in X$ 有：
$ lim_n f_n (x) = f_0 (x), $
则称 $f_n$ 弱 $*$ 收敛到 $f_0$.
]

#lemma(title: "弱收敛诱导凸收敛")[
设 ${x_n} ->^w x_0 in X$，则存在 ${y_n} subset "conv"({x_n})$ 强收敛于 $x_0$，其中 $"conv"(U)$ 表示 $U$ 的凸包。
]
#proof[
令 $K := overline("conv"({x_n}))$，则只需证明 $x_0 in K$，如不然，由 Hahn-Banach 的几何形式（凸集分离定理），存在 $f in X^*$ 使得

$ Re f(x_0) < inf_(y in K) Re f(y), $

上式左边按定义小于等于 $inf_n Re f(x_n)$，所以 $Re f(x_0) < inf_n Re f(x_n)$，与 $f(x_n) -> f(x_0)$ 矛盾！
]

#remark[
由本题证明，我们还有得到如下结果：凸闭集一定弱列闭。
]

#theorem(title: "最佳逼近元存在定理")[
设 $X$ 是自反 Banach 空间，$F subset X$ 非空且弱列闭，则对任意 $x_0 in X$，存在 $y_0 in F$ 满足
$ ||x_0 - y_0|| = "dist"(x_0, F). $
